  ...
  function(declare, html, _WidgetsInTemplateMixin, Point, SpatialReference, BaseWidget, utils, Message,
    ...
    var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      _displayOnClient: function(mapPoint) {
        ...
        if (convertInClient) {
          ...
          if (options.defaultUnit === outUnit) {
            this.coordinateInfo.innerHTML = this._toFormat(x) + "  " + this._toFormat(y);
      ...
      _setLonLat: function(position) {
        if (typeof position !== "undefined" && position.mapPoint !== null &&
          ((position.mapPoint.latitude !== null && position.mapPoint.longitude !== null) ||
          (position.mapPoint.x !== null && position.mapPoint.y !== null))
        ) {
          //with lat/lon or with x/y
          this._displayOnClient(position.mapPoint);
        } else {
      ...
      _setElev: function(position) {
        if (typeof position !== "undefined" && position.mapPoint !== null &&
          typeof position.mapPoint.z !== "undefined") {
          this._setElevInfo(position.mapPoint);
        } else {
          this._setElevInfo();
        }
      },
